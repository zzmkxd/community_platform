...
**方式一：自由加入**（待 `join_mode` 字段实现，G13 Phase 7+）
...
- 新用户通过链接加入：`POST /api/v1/servers/{id}/invites/join { code }`
...
MemberService.joinServer()
  ...
  ├── PushService.pushToServer() → WS MEMBER_JOIN 推送全服在线成员
  │   └── 异步发送系统消息到 #general 待 Phase 7+ 补充
  └── 返回 MemberVO（含角色列表）
```
...
| 6 | INVALIDATE_TOKEN | 空 — 强制重新登录 | ✅ |
| 20 | MESSAGE_CREATE | `MessageVO` — 新消息（含 attachments + reactions） | ✅ |
| 21 | MESSAGE_UPDATE | `MessageVO` — 消息编辑 | ✅ |
| 22 | MESSAGE_DELETE | `{msgId, channelId}` | ✅ |
| 23 | REACTION_ADD | `{messageId, emoji, uid, count}` | ✅ |
| 24 | REACTION_REMOVE | `{messageId, emoji, uid, count}` | ✅ |
| 25 | TYPING_START_PUSH | `{cid, tid, uid}` | ✅ |
| 26 | TYPING_STOP_PUSH | `{cid, tid, uid}` | ✅ |
| 30 | MEMBER_JOIN | `{sid, memberVO}` | ✅ `04a5877` |
| 31 | MEMBER_LEAVE | `{sid, uid}` | ✅ `04a5877` |
| 32 | MEMBER_KICK | `{sid, uid}` | ✅ `04a5877` |
| 34 | THREAD_CREATE | `ThreadVO` | ✅ |
| 40 | USER_ONLINE | `{uid}` | ✅ |
| 41 | USER_OFFLINE | `{uid}` | ✅ |
| 50 | CHANNEL_CREATE | `ChannelVO` | ✅ `04a5877` |
| 51 | CHANNEL_UPDATE | `ChannelVO` | ✅ `04a5877` |
| 52 | CHANNEL_DELETE | `{cid}` | ✅ `04a5877` |
| 53 | SERVER_UPDATE | `ServerVO` | ✅ `04a5877` |
| 99 | ERROR | `String` — 错误消息 | ✅ |

> **未实现**: SUBSCRIBE_ACK(11) 订阅确认、ROLE_UPDATE(23) 角色变更推送 — Phase 7+ 按需补充。
