...
| Phase 5 | ✅ 完成 | `7af4244` | 2026-06-22 |
| Phase 6 | 🔄 进行中 | `04a5877` | 2026-06-23 |
| Phase 7 | ⏳ 待开始 | — | — |
| Phase 8 | ⏳ 待开始 | — | — |

> **Phase 6 进度**：6.1 MinIO 文件上传 ✅ / 6.2 消息文件关联 ✅ / 6.3 Elasticsearch 搜索 ❌（待实现）
>
> **Phase 0-6 加固** (2026-06-23)：Bug 修复 (`f5f20cc`) → Redisson/频控 AOP + Caffeine 缓存 (`f5f20cc`) → G1-G4 功能缺口修复 (`6ee4354`) → WS 推送补全 8 触发点 (`04a5877`)
...
| CI/CD | GitHub Actions 编译检查 + 打包推送 GHCR（`0de68cf`） |
| 开发日志 | `docs/dev-log.md` (Phase 0-4) + `docs/dev-log-2.md` (Phase 5+) — 记录所有实现细节和决策 |
| 敏感词过滤 | AC 自动机 + DFAFilter + SensitiveWordBs（T8，11 文件） |
| EmojiMsgHandler | 表情消息策略处理器（T4） |
| MentionParser | @提及正则解析 + uid 提取（T5） |
| MinIO 文件模块 | 预签名上传 + 确认 + 下载（Phase 6.1） |
| 审计修复 | A1-A22 全量修复：端口同步、SQL 注入、getById、MinIO region 等 |
| Redisson 分布式锁 | `RedisConfig` + `RedissonLockAspect` + `LockService`（MallChat 移植） |
| 频控 AOP | `FrequencyControlAspect` + 固定窗口策略 + 策略工厂（MallChat 移植） |
| Caffeine 本地缓存 | `AbstractLocalCache` — LoadingCache 简化封装 |
| JSON TypeHandler | `MessageExtra` POJO + `JacksonTypeHandler` on `Message.extra` 字段 |
| WS 推送补全 | `PushService.pushToServer()` + 8 触发点（join/leave/kick/channel×3/server 更新） |
